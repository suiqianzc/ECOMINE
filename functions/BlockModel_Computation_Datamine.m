...
%
%  - NBM: This structure containing the complete energy consumption and GWP of each block of mining and comminution 
...
%  (1)S_op: from ore block to plant; (2)S_wd: from waste block to dump; (3)S_up: from ore block containing unmineralised materials to plant
...
%  2.Calculate energy consumption and GWP for each unit and input source(fuel/explosive/electricity) respectively and assign values to MLA structure
%  Create energy and GWP cells (ore/waste/unmineralised) for each unit
%  Drilling(Note:the unmineralised is not consider in this unit)
...
%  Temporary cells   
   ...
   for m = 1 : size(rock_code,1)
       ...
       if ~isequal(Ore(m).SEC_D1,0) && ~isempty(Ore(m).Tonnage1)
           energy_d{m,1} = Energy_Consumption(p_v.A,Ore(m).SEC_D1,p_v.L,p_v.N,p_v.nD,[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],Ore(m).Tonnage1,'Drill');
           energy_b{m,1} = Energy_Consumption([],[],[],[],[],p_v.LF,p_v.Eblast,[],[],[],[],[],[],[],[],[],[],[],[],[],[],Ore(m).Tonnage1,'Blast');
           GWP_d{m,1}    = Global_Warming_Potential(energy_d{m,1},[],p_v.EF_fuel,[],[],'Drill');
       ...
       if ~isempty(Ore(m).Tonnage1)
           ...
           GWP_l{m,1}    = Global_Warming_Potential(energy_l{m,1},[],p_v.EF_fuel,[],[],'Load');
           GWP_h{m,1}    = Global_Warming_Potential(energy_h{m,1},[],p_v.EF_fuel,[],[],'Haul');
           GWP_c{m,1}    = Global_Warming_Potential(energy_c{m,1},[],[],[],p_v.EF_electricity,'Crush');
           GWP_g{m,1}    = Global_Warming_Potential(energy_g{m,1},[],[],[],p_v.EF_electricity,'Grind');
       end
       ...
       if ~isequal(Ore(m).SEC_D1,0) && ~isempty(Ore(m).Tonnage1) %Calculate total energy consumption and GWP for each ore block
       ...
       if ~isequal(Waste(m).SEC_D2,0) && ~isempty(Waste(m).Tonnage2)
           ...
           GWP_d{m,2}    = Global_Warming_Potential(energy_d{m,2},[],p_v.EF_fuel,[],[],'Drill');
       ...
       if ~isempty(Waste(m).Tonnage2)
           ...
           GWP_l{m,2}    = Global_Warming_Potential(energy_l{m,2},[],p_v.EF_fuel,[],[],'Load');
           GWP_h{m,2}    = Global_Warming_Potential(energy_h{m,2},[],p_v.EF_fuel,[],[],'Haul');
       end
       ...
       if ~isempty(Unmine(m).Tonnage3)
           ...
           GWP_l{m,3}    = Global_Warming_Potential(energy_l{m,3},[],p_v.EF_fuel,[],[],'Load');
           GWP_h{m,3}    = Global_Warming_Potential(energy_h{m,3},[],p_v.EF_fuel,[],[],'Haul');
           GWP_c{m,3}    = Global_Warming_Potential(energy_c{m,3},[],[],[],p_v.EF_electricity,'Crush');
           GWP_g{m,3}    = Global_Warming_Potential(energy_g{m,3},[],[],[],p_v.EF_electricity,'Grind');
       end
...
%  (2)Energy consumption and GWP of each input source(fuel/explosive/electricity) of mine life cycle
   ...
   ECGWP.E_Fuel        = ECGWP.E_Drill + ECGWP.E_Load + ECGWP.E_Haul;
   ECGWP.E_Explosive   = ECGWP.E_Blast;
   ECGWP.E_Electricity = ECGWP.E_Crush + ECGWP.E_Grind;
   ECGWP.G_Fuel        = ECGWP.G_Drill + ECGWP.G_Load + ECGWP.G_Haul;
   ECGWP.G_Explosive   = ECGWP.G_Blast;
   ECGWP.G_Electricity = ECGWP.G_Crush + ECGWP.G_Grind;
...
%  Assign energy consumption and GWP of each block to this model 
   for m = 1: size(rock_code,1)
       ...
       NBM(m).Tonnage  = [Ore(m).Tonnage1 ; Waste(m).Tonnage2];
...
%  'EC_Value'    
   EC_Value(1,1) = ECGWP.E_Fuel; EC_Value(2,1) = ECGWP.E_Explosive;EC_Value(3,1) = ECGWP.E_Electricity;EC_Value(4,1) = ECGWP.E_Drill;EC_Value(5,1) = ECGWP.E_Blast;EC_Value(6,1) = ECGWP.E_Load;
   EC_Value(7,1) = ECGWP.E_Haul; EC_Value(8,1) = ECGWP.E_Crush;    EC_Value(9,1) = ECGWP.E_Grind;      EC_Value(10,1)= ECGWP.E_Drill+ECGWP.E_Blast+ECGWP.E_Load+ECGWP.E_Haul+ECGWP.E_Crush+ECGWP.E_Grind;
%  'GWP_Value'
   GWP_Value(1,1) = ECGWP.G_Fuel;GWP_Value(2,1) = ECGWP.G_Explosive;GWP_Value(3,1) = ECGWP.G_Electricity;GWP_Value(4,1) = ECGWP.G_Drill;GWP_Value(5,1) = ECGWP.G_Blast;GWP_Value(6,1) = ECGWP.G_Load;
   GWP_Value(7,1) = ECGWP.G_Haul;GWP_Value(8,1) = ECGWP.G_Crush;    GWP_Value(9,1) = ECGWP.G_Grind;      GWP_Value(10,1) = ECGWP.G_Drill+ECGWP.G_Blast+ECGWP.G_Load+ECGWP.G_Haul+ECGWP.G_Crush+ECGWP.G_Grind;
%  Build 'InputSource' struct  
   InputSource           = struct;
   InputSource.Category  = [Name(1,1);Name(2,1);Name(3,1)];
   InputSource.EC_Value  = [EC_Value(1,1);EC_Value(2,1);EC_Value(3,1)];
   InputSource.GWP_Value = [GWP_Value(1,1);GWP_Value(2,1);GWP_Value(3,1)];
...
%  Input Source(optional)
   g(1,1) = gramm('x',InputSource_table.Category,'y',InputSource_table.EC_Value/1e6,'color',InputSource_table.Category);
   g(1,1).stat_summary('geom',{'bar'},'width',0.3,'dodge',0);
   g(1,1).set_names('color','','x','','y','Energy Consumption (TJ)');
   g(1,1).set_title('Energy Consumption of Input Source','FontSize',12);
   g(1,1).no_legend();
   g(1,2) = gramm('x',InputSource_table.Category,'y',InputSource_table.GWP_Value/1e3,'color',InputSource_table.Category);
   g(1,2).stat_summary('geom',{'bar'},'width',0.3,'dodge',0);
   g(1,2).set_names('color','','x','','y',{('Global Warming Potential');('(ton CO2 eq)')});
   g(1,2).set_title('Global Warming Potential of Input Source','FontSize',12);
   g(1,2).no_legend();
%  Prcoess 
   g(2,1) = gramm('x',Process_table.Category,'y',Process_table.EC_Value/1e6,'color',Process_table.Category);
   g(2,1).stat_summary('geom',{'bar'},'dodge',0);
   g(2,1).set_color_options('map','lch');
   g(2,1).set_names('color','','x','','y','Energy Consumption (TJ)');
   g(2,1).set_title('Energy Consumption for Each Process and Whole Process','FontSize',12);
   g(2,1).no_legend();
   g(2,2) = gramm('x',Process_table.Category,'y',Process_table.GWP_Value/1e3,'color',Process_table.Category);
   g(2,2).stat_summary('geom',{'bar'},'dodge',0);
   g(2,2).set_color_options('map','lch');
   g(2,2).set_names('color','','x','','y',{('Global Warming Potential');('(ton CO2 eq)')});
   g(2,2).set_title('Global Warming Potential for Each Process and Whole Process','FontSize',12);
   g(2,2).no_legend();
   g.draw();
