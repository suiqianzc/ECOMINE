...
%  1. Annual energy consumption and GWP of each production year for optimal profile and planar profile;
%  3. Cumulative total energy consumption and GWP over the life cycle of a mine for optimal profile and planar profile.
%  2022 © C. Zhang
...
switch opt
    case 1 %GeoviaWhittle
        ...
        EC_Value  = zeros(7,2);
        GWP_Value = zeros(7,2);
        Type1     = repmat({'Optimal'},7,1);
        Type2     = repmat({'Planar'},7,1);
        %  Assign values
        %  'Name'
        ...
        for i = 1 : size(Optimal.ECGWP,2)
            %'EC_Value'
            EC_Value(1,1)  = EC_Value(1,1) + sum(Optimal.ECGWP(i).EC_Drill);
            EC_Value(2,1)  = EC_Value(2,1) + sum(Optimal.ECGWP(i).EC_Blast);
            EC_Value(3,1)  = EC_Value(3,1) + sum(Optimal.ECGWP(i).EC_Load);
            EC_Value(4,1)  = EC_Value(4,1) + sum(Optimal.ECGWP(i).EC_Haul);
            EC_Value(5,1)  = EC_Value(5,1) + sum(Optimal.ECGWP(i).EC_Crush);
            EC_Value(6,1)  = EC_Value(6,1) + sum(Optimal.ECGWP(i).EC_Grind);
            EC_Value(7,1)  = EC_Value(7,1) + sum(Optimal.ECGWP(i).Total_EC);
            %'GWP_Value'
            GWP_Value(1,1)  = GWP_Value(1,1) + sum(Optimal.ECGWP(i).GWP_Drill);
            GWP_Value(2,1)  = GWP_Value(2,1) + sum(Optimal.ECGWP(i).GWP_Blast);
            GWP_Value(3,1)  = GWP_Value(3,1) + sum(Optimal.ECGWP(i).GWP_Load);
            GWP_Value(4,1)  = GWP_Value(4,1) + sum(Optimal.ECGWP(i).GWP_Haul);
            GWP_Value(5,1)  = GWP_Value(5,1) + sum(Optimal.ECGWP(i).GWP_Crush);
            GWP_Value(6,1)  = GWP_Value(6,1) + sum(Optimal.ECGWP(i).GWP_Grind);
            GWP_Value(7,1)  = GWP_Value(7,1) + sum(Optimal.ECGWP(i).Total_GWP);
        end
        %  'Planar'
        for i = 1 : size(Planar.ECGWP,2)
            %'EC_Value'
            EC_Value(1,2)  = EC_Value(1,2) + sum(Planar.ECGWP(i).EC_Drill);
            EC_Value(2,2)  = EC_Value(2,2) + sum(Planar.ECGWP(i).EC_Blast);
            EC_Value(3,2)  = EC_Value(3,2) + sum(Planar.ECGWP(i).EC_Load);
            EC_Value(4,2)  = EC_Value(4,2) + sum(Planar.ECGWP(i).EC_Haul);
            EC_Value(5,2)  = EC_Value(5,2) + sum(Planar.ECGWP(i).EC_Crush);
            EC_Value(6,2)  = EC_Value(6,2) + sum(Planar.ECGWP(i).EC_Grind);
            EC_Value(7,2)  = EC_Value(7,2) + sum(Planar.ECGWP(i).Total_EC);
            %'GWP_Value'
            GWP_Value(1,2)  = GWP_Value(1,2) + sum(Planar.ECGWP(i).GWP_Drill);
            GWP_Value(2,2)  = GWP_Value(2,2) + sum(Planar.ECGWP(i).GWP_Blast);
            GWP_Value(3,2)  = GWP_Value(3,2) + sum(Planar.ECGWP(i).GWP_Load);
            GWP_Value(4,2)  = GWP_Value(4,2) + sum(Planar.ECGWP(i).GWP_Haul);
            GWP_Value(5,2)  = GWP_Value(5,2) + sum(Planar.ECGWP(i).GWP_Crush);
            GWP_Value(6,2)  = GWP_Value(6,2) + sum(Planar.ECGWP(i).GWP_Grind);
            GWP_Value(7,2)  = GWP_Value(7,2) + sum(Planar.ECGWP(i).Total_GWP);
        end
        %  Build 'Process' struct
        Process           = struct;
        Process.Category  = [Name(1,1);Name(2,1);Name(3,1);Name(4,1);Name(5,1);Name(6,1);Name(7,1);...
                             Name(1,1);Name(2,1);Name(3,1);Name(4,1);Name(5,1);Name(6,1);Name(1,1)];
        Process.Type      = [Type1(1,1);Type1(2,1);Type1(3,1);Type1(4,1);Type1(5,1);Type1(6,1);Type1(7,1);...
                             Type2(1,1);Type2(2,1);Type2(3,1);Type2(4,1);Type2(5,1);Type2(6,1);Type2(7,1)];
        Process.EC_Value  = [EC_Value(1,1);EC_Value(2,1);EC_Value(3,1);EC_Value(4,1);EC_Value(5,1);EC_Value(6,1);EC_Value(7,1);...
                             EC_Value(1,2);EC_Value(2,2);EC_Value(3,2);EC_Value(4,2);EC_Value(5,2);EC_Value(6,2);EC_Value(7,2)];
        Process.GWP_Value = [GWP_Value(1,1);GWP_Value(2,1);GWP_Value(3,1);GWP_Value(4,1);GWP_Value(5,1);GWP_Value(6,1);GWP_Value(7,1);...
                             GWP_Value(1,2);GWP_Value(2,2);GWP_Value(3,2);GWP_Value(4,2);GWP_Value(5,2);GWP_Value(6,2);GWP_Value(7,2)];
    ...
    case 2 %Datamine
        ...
        Name{7,1} = sprintf('Total');
        %  'EC_Value''Optimal'
        EC_Value(1,1) = Optimal.ECGWP.E_Drill;
        EC_Value(2,1) = Optimal.ECGWP.E_Blast;
        EC_Value(3,1) = Optimal.ECGWP.E_Load;
        EC_Value(4,1) = Optimal.ECGWP.E_Haul;
        EC_Value(5,1) = Optimal.ECGWP.E_Crush;
        EC_Value(6,1) = Optimal.ECGWP.E_Grind;
        EC_Value(7,1) = Optimal.ECGWP.E_Drill+Optimal.ECGWP.E_Blast+Optimal.ECGWP.E_Load...
                        +Optimal.ECGWP.E_Haul+Optimal.ECGWP.E_Crush+Optimal.ECGWP.E_Grind;
        %  'EC_Value''Planar'
        EC_Value(1,2) = Planar.ECGWP.E_Drill;
        EC_Value(2,2) = Planar.ECGWP.E_Blast;
        EC_Value(3,2) = Planar.ECGWP.E_Load;
        EC_Value(4,2) = Planar.ECGWP.E_Haul;
        EC_Value(5,2) = Planar.ECGWP.E_Crush;
        EC_Value(6,2) = Planar.ECGWP.E_Grind;
        EC_Value(7,2) = Planar.ECGWP.E_Drill+Planar.ECGWP.E_Blast+Planar.ECGWP.E_Load...
                        +Planar.ECGWP.E_Haul+Planar.ECGWP.E_Crush+Planar.ECGWP.E_Grind;
        %  'GWP_Value''Optimal'
        GWP_Value(1,1) = Optimal.ECGWP.G_Drill;
        GWP_Value(2,1) = Optimal.ECGWP.G_Blast;
        GWP_Value(3,1) = Optimal.ECGWP.G_Load;
        GWP_Value(4,1) = Optimal.ECGWP.G_Haul;
        GWP_Value(5,1) = Optimal.ECGWP.G_Crush;
        GWP_Value(6,1) = Optimal.ECGWP.G_Grind;
        GWP_Value(7,1) = Optimal.ECGWP.G_Drill+Optimal.ECGWP.G_Blast+Optimal.ECGWP.G_Load...
                         +Optimal.ECGWP.G_Haul+Optimal.ECGWP.G_Crush+Optimal.ECGWP.G_Grind;
        %  'GWP_Value''Planar'
        GWP_Value(1,2) = Planar.ECGWP.G_Drill;
        GWP_Value(2,2) = Planar.ECGWP.G_Blast;
        GWP_Value(3,2) = Planar.ECGWP.G_Load;
        GWP_Value(4,2) = Planar.ECGWP.G_Haul;
        GWP_Value(5,2) = Planar.ECGWP.G_Crush;
        GWP_Value(6,2) = Planar.ECGWP.G_Grind;
        GWP_Value(7,2) = Planar.ECGWP.G_Drill+Planar.ECGWP.G_Blast+Planar.ECGWP.G_Load...
                         +Planar.ECGWP.G_Haul+Planar.ECGWP.G_Crush+Planar.ECGWP.G_Grind;
        %  Build 'Process' struct
        Process           = struct;
        Process.Category  = [Name(1,1);Name(2,1);Name(3,1);Name(4,1);Name(5,1);Name(6,1);Name(7,1);...
                             Name(1,1);Name(2,1);Name(3,1);Name(4,1);Name(5,1);Name(6,1);Name(7,1)];
        Process.Type      = [Type1(1,1);Type1(2,1);Type1(3,1);Type1(4,1);Type1(5,1);Type1(6,1);Type1(7,1);...
                             Type2(1,1);Type2(2,1);Type2(3,1);Type2(4,1);Type2(5,1);Type2(6,1);Type2(7,1)];
        Process.EC_Value  = [EC_Value(1,1);EC_Value(2,1);EC_Value(3,1);EC_Value(4,1);EC_Value(5,1);EC_Value(6,1);EC_Value(7,1);...
                             EC_Value(1,2);EC_Value(2,2);EC_Value(3,2);EC_Value(4,2);EC_Value(5,2);EC_Value(6,2);EC_Value(7,2)];
